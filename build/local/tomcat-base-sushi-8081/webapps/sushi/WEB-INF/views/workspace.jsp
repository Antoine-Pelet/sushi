...
<header class="topbar">
    ...
    <div class="top-meta">
        <a class="cart-link" href="<%= currentUser != null ? "#cart-panel" : ctx + "/auth?returnPage=workspace&returnSection=cart-panel" %>">🧺<% if (cartCount > 0) { %><span class="cart-badge"><%= cartCount %></span><% } %></a>
        <a class="account-photo" href="<%= ctx %>/auth" aria-label="<%= currentUser != null ? "Compte " + esc(currentUser.getUsername()) : "Compte" %>"><span><%= esc(initials(currentUser)) %></span></a>
    </div>
</header>
    ...
    <section class="screen screen--utility">
        <div class="utility-grid">
            <article class="utility-card" id="auth-panel">
                <div class="section-head"><div><p class="eyebrow">Compte</p><h2><%= currentUser == null ? "Connexion / inscription" : "Session active" %></h2></div></div>
                <% if (currentUser == null) { %>
                <p class="helper">Une page dediee vous permet maintenant de vous connecter ou de creer un compte sans quitter l'univers visuel de Sushef.</p>
                <div class="auth-actions">
                    <a class="button button--outline" href="<%= ctx %>/auth?returnPage=workspace&returnSection=auth-panel">Se connecter</a>
                    <a class="button button--ghost" href="<%= ctx %>/auth?mode=register&returnPage=workspace&returnSection=auth-panel#register-card">Creer un compte</a>
                </div>
                ...
                <div class="auth-actions"><a class="button button--ghost" href="<%= ctx %>/auth">Gerer mon compte</a><form method="post" action="<%= ctx %>/auth/logout"><input type="hidden" name="returnPage" value="workspace"><input type="hidden" name="returnSection" value="catalogue"><button type="submit" class="button button--ghost">Se deconnecter</button></form></div>
                <% } %>
            </article>
            ...
            <article class="utility-card">
                ...
                <% if (currentUser != null) { %>
                    ...
                    <% } %>
                <% } else { %>
                    <div class="api-list"><code>GET <%= ctx %>/api/recipes</code><code>GET <%= ctx %>/api/me</code><code>POST <%= ctx %>/api/session/login</code><code>POST <%= ctx %>/api/session/register</code><code>POST <%= ctx %>/api/cart/add</code><code>POST <%= ctx %>/api/orders/checkout</code></div>
                <% } %>
            </article>
        </div>
    </section>
